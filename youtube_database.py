...
t1 = datetime.datetime(2016, 11, 24, 0, 0, 0)
t1 = calendar.timegm(t1.timetuple())
t2 = datetime.datetime(2017, 1, 31, 0, 0, 0)
            ...
            while True:
                ...
                for r in search.get("items", []):
                    ...
                    videos = youtube.videos().list(
                        id=videoId,
                        # id, snippet, contentDetails, localizations, player, statistics, status
                        part="snippet, contentDetails, recordingDetails, statistics, status",
                    ).execute()

                    for r in videos.get("items", []):
                        # skip live broadcasts
                        try:
                            liveBroadcastContent = r["snippet"][
                        ...
                        publishedAt = int(
                            strict_rfc3339.rfc3339_to_timestamp(publishedAt))
                        currentTime = datetime.datetime.utcnow()  # current time as rtf3339
                        currentTime = datetime.datetime.timetuple(
                            currentTime)  # current time as timetuple
                        # current time as epoch timestamp
                        currentTime = calendar.timegm(currentTime)
                        life = currentTime - publishedAt

                        # contentDetails
                        definition = r["contentDetails"]["definition"]
                        caption = 0 if r["contentDetails"][
                            "caption"].lower() == 'false' else 1
                        duration = r["contentDetails"]["duration"]
                        duration_w = re.search(r"(\d+)w", duration, re.I)
                        duration_w = int(duration_w.group(1)
                                         ) if duration_w else 0
                        duration_d = re.search(r"(\d+)d", duration, re.I)
                        duration_d = int(duration_d.group(1)
                                         ) if duration_d else 0
                        duration_h = re.search(r"(\d+)h", duration, re.I)
                        duration_h = int(duration_h.group(1)
                                         ) if duration_h else 0
                        duration_m = re.search(r"(\d+)m", duration, re.I)
                        duration_m = int(duration_m.group(1)
                                         ) if duration_m else 0
                        duration_s = re.search(r"(\d+)s", duration, re.I)
                        duration_s = int(duration_s.group(1)
                                         ) if duration_s else 0
                        duration = 0
                        ...
                        licensedContent = 0 if r["contentDetails"][
                            "licensedContent"] == False else 1
                        dimension = r["contentDetails"]["dimension"]
                        try:
                            allowed = ','.join(r["contentDetails"][
                                               "regionRestriction"]["allowed"])
                        except Exception:
                            allowed = None
                        try:
                            allowedCount = len(r["contentDetails"][
                                               "regionRestriction"]["allowed"])
                        except Exception:
                            allowedCount = 0
                        ...
                        # recordingDetails
                        try:
                            recordingDate = r["recordingDetails"][
                                "recordingDate"]
                            recordingDate = int(
                                strict_rfc3339.rfc3339_to_timestamp(recordingDate))
                        except Exception:
                            recordingDate = None
                        try:
                            latitude = r["recordingDetails"][
                                "location"]["latitude"]
                        except Exception:
                            latitude = None
                        try:
                            longitude = r["recordingDetails"][
                                "location"]["longitude"]
                        except Exception:
                            longitude = None
                        ...
                        try:
                            db.execute(
                                "INSERT INTO VIDEOS VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (sql_insert_vars))
                            # print "Record inserted successfully";
                        except sqlite3.IntegrityError:
                            print('sqlite3.IntegrityError: videoId=%s' %
                                  videoId)

                        # --- %s              , title.encode('utf-8')
                        print("%s - %s --- #%d --- %s" % (publishedAfter,
                                                          publishedBefore, idx, videoId.encode('utf-8')))
                        idx += 1
